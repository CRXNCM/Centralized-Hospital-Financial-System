import { useMemo } from "react";
import { formatEtb } from "../../data/receptionBills";
import { PAYMENT_DISPLAY_STATUS } from "../../data/receptionBillStore";
import { useReceptionBills } from "../../context/ReceptionBillsContext";
import { IconCheck, IconClock, IconUsers } from "../icons";
import { getPaymentDisplayStatus } from "./PaymentStatusBadge";

export default function ReceptionHome({ onRecordPayment }) {
  const { unpaidBills, todaysPayments } = useReceptionBills();

  const stats = useMemo(() => {
    const paidToday = todaysPayments.filter(
      (p) => getPaymentDisplayStatus(p) === PAYMENT_DISPLAY_STATUS.PAID,
    );
    return {
      paymentsRecorded: paidToday.length,
      pendingBills: unpaidBills.length,
      totalCollected: paidToday.reduce((sum, p) => sum + p.amount, 0),
    };
  }, [todaysPayments, unpaidBills.length]);

  const queue = useMemo(
    () =>
      unpaidBills.slice(0, 8).map((bill) => ({
        name: bill.patientName,
        service: bill.service,
        billId: bill.billId,
        amount: bill.amount,
      })),
    [unpaidBills],
  );

  const deskStats = [
    {
      label: "Payments Today",
      value: String(stats.paymentsRecorded),
      icon: IconCheck,
      color: "text-[#10B981]",
    },
    {
      label: "Collected Today",
      value: formatEtb(stats.totalCollected),
      icon: IconUsers,
      color: "text-[#22D3EE]",
    },
    {
      label: "Pending Bills",
      value: String(stats.pendingBills),
      icon: IconClock,
      color: "text-[#F59E0B]",
    },
  ];

  return (
    <div className="space-y-6 p-8">
      <div className="grid grid-cols-1 gap-5 sm:grid-cols-3">
        {deskStats.map((stat) => {
          const Icon = stat.icon;
          return (
            <article key={stat.label} className="glass-card p-5">
              <div className="flex items-start justify-between">
                <div>
                  <p className="text-sm text-[#94A3B8]">{stat.label}</p>
                  <p className={`mt-2 text-2xl font-bold ${stat.color}`}>{stat.value}</p>
                </div>
                <div className="rounded-xl bg-[rgba(34,211,238,0.08)] p-2">
                  <Icon className="h-5 w-5 text-[#22D3EE]" />
                </div>
              </div>
            </article>
          );
        })}
      </div>

      <section className="glass-card overflow-hidden">
        <div className="border-b border-[rgba(34,211,238,0.1)] px-6 py-4">
          <h2 className="font-semibold text-white">Waiting Queue</h2>
          <p className="text-sm text-[#94A3B8]">Unpaid bills ready for payment</p>
        </div>
        {queue.length === 0 ? (
          <p className="px-6 py-10 text-center text-sm text-[#64748B]">
            No unpaid bills in the queue.
          </p>
        ) : (
          <ul className="divide-y divide-[rgba(34,211,238,0.06)]">
            {queue.map((item) => (
              <li
                key={item.billId}
                className="flex flex-wrap items-center justify-between gap-3 px-6 py-4 transition-colors hover:bg-[rgba(34,211,238,0.04)]"
              >
                <div>
                  <p className="font-medium text-white">{item.name}</p>
                  <p className="text-sm text-[#94A3B8]">{item.service}</p>
                </div>
                <div className="flex items-center gap-4">
                  <span className="text-sm font-semibold text-[#22D3EE]">
                    {formatEtb(item.amount)}
                  </span>
                  <span className="font-mono text-xs text-[#94A3B8]">{item.billId}</span>
                  <span className="badge-pending rounded-full px-2.5 py-0.5 text-xs font-semibold">
                    Unpaid
                  </span>
                </div>
              </li>
            ))}
          </ul>
        )}
      </section>

      {onRecordPayment && (
        <button
          type="button"
          onClick={onRecordPayment}
          className="btn-confirm-active rounded-xl px-6 py-3.5 text-sm font-bold"
        >
          Record Payment
        </button>
      )}
    </div>
  );
}
