...
export default function PaymentConfirmScreen({ payment, onBack, onComplete }) {
  const { bill, method, methodId, amount } = payment;
  const [loading, setLoading] = useState(false);
  const MethodIcon = getPaymentMethodIcon(methodId);
  ...
  return (
    <div className="mx-auto w-full max-w-xl space-y-6">
      <header className="text-center sm:text-left">
        <h1 className="text-2xl font-bold text-white">Confirm Payment Details</h1>
        <p className="mt-1 text-sm text-[#94A3B8]">Please review before confirming</p>
      </header>

      <article className="glass-card p-6 sm:p-8">
        <div className="divide-y divide-[rgba(34,211,238,0.08)]">
          <SummaryRow label="Patient Name">{bill.patientName}</SummaryRow>
          <SummaryRow label="Bill ID">{bill.billId}</SummaryRow>
          <SummaryRow label="Service Received">{bill.service}</SummaryRow>
          <SummaryRow label="Payment Method">
            <span className="inline-flex items-center justify-end gap-2">
              <MethodIcon className="h-5 w-5 text-[#22D3EE]" />
              <span className="font-semibold text-[#22D3EE]">{method}</span>
            </span>
          </SummaryRow>
        </div>
        ...
        <div className="mt-6 rounded-xl border border-[rgba(34,211,238,0.2)] bg-[rgba(5,13,26,0.4)] px-5 py-5 text-center">
          ...
          <p className="mt-2 text-3xl font-bold text-white sm:text-4xl">{formatEtb(amount)}</p>
          {amount !== bill.amount && (
            <p className="mt-2 text-sm text-[#F59E0B]">
              Bill total was {formatEtb(bill.amount)}
            </p>
          )}
        </div>
      </article>
