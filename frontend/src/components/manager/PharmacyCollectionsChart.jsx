...
export default function PharmacyCollectionsChart() {
  const [period, setPeriod] = useState("daily");
  const [animateKey, setAnimateKey] = useState(0);
  const [hoveredBar, setHoveredBar] = useState(null);
  const [staff, setStaff] = useState(() =>
    buildPharmacyCollections("daily", PHARMACY_COLLECTIONS.daily),
  );

  const maxAmount = Math.max(...staff.map((s) => s.amount), 1);
  const totalAmount = staff.reduce((sum, s) => sum + s.amount, 0);
  const periodLabel = CHART_PERIOD_LABELS[period];

  const refresh = useCallback(() => {
    setStaff(buildPharmacyCollections(period, PHARMACY_COLLECTIONS[period]));
  }, [period]);
  ...
  return (
    <section className="glass-card p-6">
      <div className="mb-6 flex flex-wrap items-end justify-between gap-4">
        <div>
          <h2 className="text-lg font-semibold text-white">Collections by Pharmacy</h2>
          <p className="mt-1 text-sm text-[#94A3B8]">
            Pharmacy sales recorded per staff member — {periodLabel}
          </p>
        </div>
        <div className="flex flex-wrap items-center gap-3">
          <div className="flex rounded-xl border border-[rgba(139,92,246,0.2)] bg-[rgba(5,13,26,0.5)] p-1">
            {PERIODS.map((p) => (
              <button
                key={p}
                type="button"
                ...
                className={`rounded-lg px-3 py-1.5 text-xs font-semibold capitalize transition-all ${
                  period === p
                    ? "bg-[#8B5CF6] text-white shadow-[0_0_12px_rgba(139,92,246,0.45)]"
          ...
          {staff.map((person, index) => {
            ...
            return (
              ...
              >
                ...
                >
                  {isHovered && (
                    <div className="chart-tooltip absolute bottom-full left-1/2 z-10 mb-2 -translate-x-1/2 whitespace-nowrap rounded-lg border border-[rgba(139,92,246,0.35)] bg-[#112240] px-3 py-2 text-xs shadow-lg">
                      <p className="font-semibold text-white">{person.name}</p>
                      <p className="mt-0.5 font-semibold text-[#A78BFA]">
                        {formatEtb(person.amount)}
                      </p>
                      <p className="mt-0.5 text-[#94A3B8]">
                        {person.transactions} sales · {share}%
                      </p>
      ...
      <div className="mt-6 flex flex-wrap justify-center gap-6 border-t border-[rgba(139,92,246,0.12)] pt-4 sm:justify-start">
        {staff.map((person) => (
          <div key={person.name} className="flex items-center gap-2">
            ...
            <span className="text-xs text-[#94A3B8]">
              {person.name}
              <span className="ml-1 text-[#64748B]">({person.transactions} sales)</span>
            </span>
          </div>
        ))}
      </div>
